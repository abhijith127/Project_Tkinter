...
class InstaClone:
    def _init_(self, root):
        ...
        header = tk.Label(root, text="📸 SVYASA DEEMED TO BE UNIVERSITY", font=("Arial", 22, "bold"),bg="#67efef", fg="#124ae4")
        header.pack(pady=10)
        ...
        self.canvas = tk.Canvas(root, bg="#f1b4ee", width=520, height=500)
    ...
    def display_post(self, img, caption):
        ...
        like_btn = tk.Button(action_frame, text="❤ Like", command=lambda:self.like_action(like_btn), bg="#fff", border=0)
    ...
    def like_action(self, button):
        if button["text"] == "❤ Like":
            button.config(text="❤ Liked", fg="#e1306c")
        else:
            button.config(text="❤ Like", fg="black")
